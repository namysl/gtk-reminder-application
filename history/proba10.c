...
/*
void replace_line(int line, char *path){
    FILE * fPtr;
    FILE * fTemp;
//    char path[100];
...
void event_alert(unsigned i)
{
    ...
    gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(event),
                                             "%s\n%s\n%s",
                                             record[i].description, record[i].date, record[i].time);
    ...
    if(response==-5){
        record[i].clicked = 1;
        //replace_line(i, "baza_clicked.txt");
    }
...
/*
    ...
    */
    ...
    for(int i=0; i<=count-1; i++){
        printf("%d, %d, %d, %d, %d, %d", record[i].clicked, record[i].y, record[i].M, record[i].d, record[i].h, record[i].m);
        puts(record[i].date);
        puts(record[i].time);
...
void add_new_entry(GtkWidget *widget, gint response_id, gpointer data){
    if(response_id==-5){
        ...
        notification("Przypominacz:", "anulowano", "face-monkey-symbolic");
    }
}
