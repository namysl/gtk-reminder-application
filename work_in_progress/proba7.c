...
/*
void show_new_entry(GtkWidget *widget, gpointer data){
    event_struct foo;
    ...
    if(response_id==-5){
        ...
        if(filehandle == NULL){
            ...
            if ((validate_date(y, M, d)==0) && (validate_localdatetime(y, M, d, h, m))==0){
                ...
                int last = load_from_file();

                model = gtk_tree_view_get_model(treeview);
                gtk_list_store_insert(GTK_LIST_STORE(model), &iter, -1);

                puts("COS NIE TAK:");
                puts(record[-1].description);
                gtk_list_store_set(GTK_LIST_STORE (model), &iter,
                                   COLUMN_DATE, *record[last].date,
                                   COLUMN_TIME, *record[last].time,
